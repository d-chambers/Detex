...
class SubSpaceStream(object):
    ...
    # make sure user defined values are acceptable
    ...
    def SVD(self, selectCriteria=2, selectValue=0.9, conDatNum=100,
        ...
        for station in self.ssStations:  # Iterate through all subspaces defined by cluster object
            for row in self.subspaces[
                    station].iterrows():  # iterate through all stations in the subspace
                self.subspaces[station].UsedSVDKeys[row[0]] = []
                svdDict = {}  # initialize empty dict to put all SVD vects in
                keys = sorted(row[1].Events)
                Arr, basisLength = self._trimGroups(
                    row, keys, station)  # get trimmed groups
                if normalize:
                    ArrNorm = np.array([x / np.linalg.norm(x) for x in Arr])
                    U, s, Vh = scipy.linalg.svd(
                        np.transpose(ArrNorm), full_matrices=False)  # perform SVD
                else:
                    try:
                        U, s, Vh = scipy.linalg.svd(
                            np.transpose(Arr), full_matrices=False)
                    except:
                        deb([station, row, Arr, keys])
                for eival in range(
                        len(s)):  # make dictionary of SVD with singular value as key and basis vector as value
                ...
                self.subspaces[station].UsedSVDKeys[
                    row[0]] = self._getUsedBasis(
                    row, svdDict, self.subspaces[station].FracEnergy[
                        row[0]], selectCriteria, selectValue)
                self.subspaces[station].SVDdefined[row[0]] = True
                self.subspaces[station].NumBasis[row[0]] = len(
                    self.subspaces[station].UsedSVDKeys[row[0]])
    ...
    def _setThresholds(
            ...
            basisLength):
        if Threshold > 0:
            for a, station in enumerate(
                    self.ssStations):  # Iterate through all subspaces defined by cluster object
            ...
            for a, station in enumerate(self.ssStations):
                ...
                detex.log(
                    __name__,
                    'Estimating effective dimesion of signal space for subspace %d' %
                    a,
                    pri=1)
                sigdimreps = _estimateDimSpace(
                    subspa,
                    conDatNum,
                    self.clusters,
                    ConDir=self.condir,
                    dtype=self.dtype)  # estiamte dimension of signal space
                Thresholds = []
                for badim, sigdim in zip(basisdims, sigdimreps):
                    # reworking eqaution 20 of Harris 2006 should yeild this
                    # expression
                    Thresholds.append(
                        [1 / ((scipy.stats.f.isf(self.Pf, dfn=badim, dfd=sigdim - badim) * (badim) / (sigdim - badim))**(-1) - 1)])
            ...
            for a, station in enumerate(self.ssStations):
                ...
                for rownum, row in subspa.iterrows():
                    ...
                    if TH > .9:
                        TH, Pftemp = self._approximateThreshold(
                            beta_a, beta_b, self.Pf, 1000, 3)

                        detex.log(
                            __name__,
                            'Scipy.stats.beta.isf failed with pf=%e, approximated Threshold to %f with a Pf of %e for station %s %s using forward grid search' %
                            (self.Pf,
                             TH,
                             Pftemp,
                             station,
            ...
            for a, station in enumerate(self.ssStations):
                subspa = self.subspaces[station]
                for rownum, row in subspa.iterrows():
                    self.subspaces[station].Threshold[rownum] = row.FracEnergy[
                        'Minimum'][row.NumBasis] * selectValue

    def _approximateThreshold(
            self,
            beta_a,
            beta_b,
            target,
        ...
        while loops < numloops:
            ...
            if minind == 0 or minind == numintervals - 1:
                detex.log(
                    __name__,
                    'Grind search failing, set threshold manually',
                    level='warning')
    ...
    def _trimGroups(self, row, keys, station):  # basic function to get trimed subspaces
        if 'Starttime' in row[1].SampleTrims.keys() and 'Endtime' in row[
                1].SampleTrims.keys():
            ...
            if stim < 0:  # make sure stim is not less than 0 #TODO impliment more robust fix
                stim = 0
            Arr = np.vstack([row[1].AlignedTD[x][stim:etim] -
                             np.mean(row[1].AlignedTD[x][stim:etim]) for x in keys])
            basisLength = Arr.shape[1]
        else:
            detex.log(
                __name__, 'No trim times for %s and station %s, try running pickSubSpaceTimes' %
                (row[1].Name, station), pri=1)
            Arr = np.vstack([row[1].AlignedTD[x] -
                             np.mean(row[1].AlignedTD[x]) for x in keys])
            basisLength = Arr.shape[1]
...
#        return threshs
    ...
    # calculate the percentage of energy that is repressented for each mp
    # template in the SVD basis
    def _getFracEnergy(self, row, svdDict, U):
        fracDict = {}
        keys = row[1].Events
        svales = svdDict.keys()
        ...
        for key in keys:
            if 'Starttime' in row[1].SampleTrims.keys() and 'Endtime' in row[
                ...
                aliwf = row[1].AlignedTD[key][
                    row[1].SampleTrims['Starttime']:row[1].SampleTrims['Endtime']]
            else:
                aliwf = row[1].AlignedTD[key]
            try:
                repvect = np.insert(
    ...
    def plotBasisVectors(self, onlyused=True):
        ...
        if not self.subspaces.values()[0].iloc[0].SVDdefined:
            raise Exception(
                'SVD not yet defined, call SVD before plotting basis vectors')
        ...
        for station in self.ssStations:
            for num, row in self.subspaces[station].iterrows():
                if len(row.SampleTrims.keys()) < 1:
                    raise Exception(
                        'subspaces must be trimmed before calling this method')
                plt.figure(count)
    ...
    def _pickTimes(
        ...
        for station in trdfDict.keys():
            for starow in trdfDict[station].iterrows():
                if not starow[
                        1].SampleTrims or repick:  # If the sample trim dictionary is empty
                    TR = self._makeOpStream(
                        starow, traceLimit)  # Make an obspy stream
                    ...
                    if d1:  # if any picks made
                        sr = starow[1].Stats[starow[1].Events[0]][
                            'sampling_rate']  # fetch sampling rate
                        Nc = starow[1].Stats[starow[1].Events[0]][
                            'Nc']  # fetch number of channels
                        # get sample divisible by NC to keep traces aligned
                        d1['Starttime'] = int(
                            min(d1.values())) - (int(min(d1.values()))) % Nc
                        # if duration paramenter is defined (it is usually
                        # better to leave it defined)
                        if duration:
                            d1['Endtime'] = d1['Starttime'] + \
                                int(duration * sr * Nc)
                            d1['DurationSeconds'] = duration
                        else:
                            d1['Endtime'] = int(max(d1.values()))
                            d1['DurationSeconds'] = (
                                d1['Endtime'] - d1['Starttime']) / (sr * Nc)
                        trdfDict[station].SampleTrims[starow[0]] = d1
                        for event in starow[1].Events:  # update starttimes
                            stime = trdfDict[station].Stats[
                                starow[0]][event]['starttime']
                            trdfDict[station].Stats[starow[0]][event][
                                'starttime'] = stime + d1['Starttime'] / (Nc * sr)
                            trdfDict[station].Stats[starow[0]][event]['offset'] = trdfDict[station].Stats[starow[
                                0]][event]['starttime'] - trdfDict[station].Stats[starow[0]][event]['origintime']
                    if not Pks.KeepGoing:
                        detex.log(
                            __name__, 'aborting picking, progress saved', pri=1)
                        return None
            self._updateOffsets()
    ...
    def attachPickTimes(
        ...
        try:  # read pksFile
            ...
            try:
                ...
                detex.log(
                    __name__,
                    '%s does not exist, or it is not a pkl or csv file' %
                    pksFile,
                    level='warning')
                raise Exception(
                    '%s does not exist, or it is not a pkl or csv file' %
        ...
        if ssMode == 'Average':
            ...
            detex.log(
                __name__,
                'ssMode %s not supported, options are: Average,Max,Min,Median' %
                ssMode,
                level='warning')
            raise Exception(
                'ssMode %s not supported, options are: Average,Max,Min,Median' %
        ...
        for cl in self.clusters:  # loop through each station in cluster object, get singles and subspaces
            ...
            if sta in self.singles.keys():
                for num, row in self.singles[sta].iterrows():
                    if len(row.SampleTrims.keys()
                           ) > 0:  # skip if sampletrims already defined
                        continue
    ...
    def _getSampleTrims(
        ...
        for ev in eves:  # loop through each event
            ...
            DF.Stats[num][ev]['offset'] = start - \
                DF.Stats[num][ev]['origintime']
        if len(startsamps) > 0:
            outdict = {
                'Starttime': int(
                    fun(startsamps)) - int(
    ...
    def _getStats(self, row):
        ...
        if len(set(sr)) != 1:
            detex.log(
                __name__,
                'Events %s on Staion %s have different sampling rates or no sampling rates' %
                (row.Station,
                 row.events),
                level='warning')
            raise Exception(
                'Events %s on Staion %s have different sampling rates or no sampling rates' %
                (row.Station, row.events))

        Nc = [row.Stats[x]['Nc'] for x in eves]
        if len(set(Nc)) != 1:
            detex.log(
                __name__,
                'Events %s on Staion %s have different numbers of channels or no channels' %
                (row.Station,
                 row.events),
                level='warning')
            raise Exception(
    ...
    def setSinglesThresholds(self, conDatNum=50, recalc=False, Threshold=None):
        ...
        for sta in self.singStations:
            self.singles[sta] = self.singles[sta][[len(x.keys()) > 0 for x in self.singles[
                sta].SampleTrims]]  # delete any singles that do not have pick times
            self.singles[sta].reset_index(inplace=True, drop=True)
            self.singles[sta].Name = ['SG%d' %
                                      x for x in range(len(self.singles[sta]))]
        if not Threshold:
            # get empirical dist unless manual threshold is passed
            self.getFAS(conDatNum, useSingles=True, useSubSpaces=False)
        for sta in self.singStations:
            for num, row in self.singles[sta].iterrows():
                ...
                if Threshold:
                    ...
                    if TH > .9:
                        TH, Pftemp = self._approximateThreshold(
                            beta_a, beta_b, self.Pf, 1000, 3)
                        # print 'Scipy.stats.beta.isf failed with pf=%e,
                        # approximated Threshold to %f with a Pf of %e for
                        # station %s %s using forward grid
                        # search'%(self.Pf,TH,Pftemp,row.Station,row.Name)
    ...
    def getFAS(
        ...
        if useSubSpaces:
            ...
            for station in self.subspaces.keys():
                if isinstance(
                        self.subspaces[station]['FAS'][0],
                        dict) and not recalc:
                    print(
                        'FAS for station %s already calculated, to recalculate pass True to the parameter recalc' %
                        station)
                else:
                    self.subspaces[station]['FAS'] = detex.fas._initFAS(
                        self.subspaces[station],
                        conDatNum,
                        self.clusters,
                        self.cfetcher,
                        LTATime=LTATime,
        ...
        if useSingles:
            ...
            for station in self.singles.keys():
                for a in range(len(self.singles[station])):
                    if isinstance(
                            self.singles[station]['FAS'][a],
                            dict) and not recalc:
                        print(
                            'FAS for single event %d already calculated, to recalculate pass True to the parameter recalc' %
                            a)
                    # skip any events that have not been trimmed
                    elif len(self.singles[station]['SampleTrims'][a].keys()) < 1:
                        continue
                    else:
                        self.singles[station]['FAS'][a] = detex.fas._initFAS(
                            self.singles[station][
                                a:a + 1],
                            conDatNum,
                            self.clusters,
                            self.cfetcher,
    ...
    def detex(
        ...
        if multiprocess or trigCon != 0:  # make sure no parameters that dont work yet are selected
            detex.log(
                __name__,
                'multiprocessing and trigcon other than 0 not yet supported',
                level='warning')
            raise Exception(
                'multiprocessing and trigcon other than 0 not yet supported')

        if os.path.exists(
                subspaceDB) and not delOldCorrs:  # If old database is around delete it
        ...
        if useSubSpaces:
            TRDF = self.subspaces
            if not all([y['SVDdefined'] for x in TRDF.keys() for num, y in self.subspaces[
                       x].iterrows()]):  # make sure SVD has been performed
                raise Exception(
                    'subspace not yet defined, call SVD before attempting to run subspace detectors')
            Det = SSDetex(
                TRDF,
                UTCstart,
                UTCend,
        ...
        if useSubSpaces or useSingles:  # save addational info to sql database
            ...
            dffilt = pd.DataFrame([self.clusters.filt], columns=[
                                  'FREQMIN', 'FREQMAX', 'CORNERS', 'ZEROPHASE'], index=[0])  # save filter info
    ...
    def _getInfoDF(self):
        ...
        for sta in self.Stations:
            if sta not in self.ssStations:
                continue
            for num, ss in self.subspaces[
                    sta].iterrows():  # write the subspace info
                name = ss.Name
                station = ss.Station
                ...
                sslist.append(pd.DataFrame([[name, station, events, thresh, numbasis, beta1, beta2]], columns=[
                              'Name', 'Sta', 'Events', 'Threshold', 'NumBasisUsed', 'beta1', 'beta2']))
            if sta not in self.singStations:
                continue
            for num, ss in self.singles[
                    sta].iterrows():  # write the singles info
                name = ss.Name
                station = ss.Station
                events = ','.join(ss.Events)
                thresh = ss.Threshold
                if len(ss.FAS[0].keys()) > 1:
                    beta1, beta2 = ss.FAS[0]['betadist'][
                        0], ss.FAS[0]['betadist'][1]
                else:
                    beta1, beta2 = np.Nan, np.Nan
    ...
    def _getHistograms(self, useSubSpaces, useSingles):
        ...
        """
        if useSubSpaces:
            sshists = [pd.DataFrame([['Bins', 'Bins', json.dumps(
                self.histSubSpaces['Bins'].tolist())]], columns=['Name', 'Sta', 'Value'])]
            for sta in self.Stations:
                if sta in self.histSubSpaces.keys():
                    for skey in self.histSubSpaces[sta]:
                        sshists.append(pd.DataFrame([[skey, sta, json.dumps(self.histSubSpaces[
                                       sta][skey].tolist())]], columns=['Name', 'Sta', 'Value']))

            sshist = pd.concat(sshists, ignore_index=True)
        else:
            sshist = None
        if useSingles:
            sghists = [pd.DataFrame([['Bins', 'Bins', json.dumps(
                self.histSingles['Bins'].tolist())]], columns=['Name', 'Sta', 'Value'])]
            for sta in self.Stations:
                if sta in self.histSingles.keys():
                    for skey in self.histSingles[sta]:
                        sghists.append(pd.DataFrame([[skey, sta, json.dumps(self.histSingles[
                                       sta][skey].tolist())]], columns=['Name', 'Sta', 'Value']))
    ...
    def __getitem__(self, key):  # make object indexable
        if isinstance(key, int):
            ...
            if len(key.split('.')) == 2:
                ...
                detex.log(
                    __name__,
                    '%s is not a station in this cluster object' %
                    key,
                    level='warning')
                raise Exception(
                    '%s is not a station in this cluster object' %
            ...
            detex.log(
                __name__,
                '%s must either be a int or str of station name' %
                key,
                level='warning')
            raise Exception(
                '%s must either be a int or str of station name' %
...
class SSDetex(object):
    ...
    def __init__(
        ...
        if isinstance(
                UTCSaves,
                collections.Iterable):  # if using UTCsave list initialize empty list
            self.UTCSaveList = []
            self.UTCSaves = np.array(
                [obspy.core.UTCDateTime(x).timestamp for x in UTCSaves])
        jobs = []
        ...
        for sta in stations:  # loop through each station in all subspaces/singles
            # make data frame from all subspaces/singles that share a station.
            # This is done to reduce IO costs in reading continuous data
            DFsta = TRDF[sta]
            DFsta.reset_index(inplace=True, drop=True)
            if len(DFsta) > 0 and not self.eventDir:
                self.hist[sta] = [None] * len(TRDF)
                if multiprocess:  # TODO fix this, multiprocessing is broken
                    p = multiprocessing.Process(
                        target=self._CorStations(DFsta, sta))
                    jobs.append(p)
                    p.start()
                else:
    ...
    def _CorStations(self, DFsta, sta):
        ...
        channels = sorted(
            set([z for x in DFsta.Channels for y in x.values() for z in y]))
        # get sampling rates
        samplingRate = list(set([y['sampling_rate']
                                 for x in DFsta.Stats for y in x.values()]))
        Names = DFsta.Name.values
        # dictionary of required coeficients
        threshold = {x.Name: x.Threshold for num, x in DFsta.iterrows()}
        Names.sort()
        histdic = {na: [0.0] * (len(self.hist['Bins']) - 1) for na in Names}
        # Make sure there is only one sampling rate for all subspaces using
        # current station station
        if len(samplingRate) > 1:
            detex.log(
                __name__,
                'More than one sampling rate found for station %s in subspace detection, aborting' %
                DFsta.Station[0],
                level='warning')
            raise Exception(
                'More than one sampling rate found for station %s in subspace detection, aborting' %
            ...
            try:
                ...
                return
                # deb([DFsta,sta])
        # get the ammount that each continuous data sample needs to be trimmed
        # before performing ss detections
        try:
            contrim = {x[1].Name: -
                       (self.continuousDataLength %
                        3600) +
                       (x[1].SampleTrims['Endtime'] -
                        x[1].SampleTrims['Starttime']) /
                       (samplingRate *
                        len(channels)) for x in DFsta.iterrows()}
        except:
            deb([DFsta, sta, self])
        staPksDf = None
        if self.pks:
    ...
    def _CorEventDat(
        ...
        if len(tempFiles) < 1:
            detex.log(
                __name__, 'No events found for %s' %
                sta, level='warning')
        else:
            ssArrayTD, ssArrayFD, reqlen, offsets, mags, eventWaveForms, events, WFU, UtU = self._loadMPSubSpace(
                DFsta, self.continuousDataLength, sta, channels, samplingRate, returnFull=True, PKS=staPksDf)
            for fileToCorr in tempFiles:  # loop through each chunk of continuous data
                CorDF, MPcon, ConDat = self._getRA(
                    ssArrayTD, ssArrayFD, fileToCorr, len(channels), reqlen, contrim, Names)
                if not isinstance(
                        CorDF, pd.DataFrame):  # if something is broken skip hour
                    detex.log(
                        __name__,
                        '%s failed' %
                        fileToCorr,
                ...
                for name, row in CorDF.iterrows(
                    ...
                    self.eventCorList.append(pd.DataFrame([[sta, name, row.File, row.MaxDS, row.MaxSTALTA]], columns=[
                                             'Station', 'Subspace', 'File', 'DS', 'DS_STALTA']))
                    if self.calcHist and len(
                            CorDF) > 0:  # If calculating histogram of Statistic of detection
                        try:
                            histdic[name] = histdic[
                                name] + np.histogram(row.SSdetect, bins=self.hist['Bins'])[0]
                        except:
                            detex.log(
                                __name__, 'binning failed for %s' %
                                row.Name, level='warning')
                    if isinstance(self.UTCSaves, collections.Iterable):
                        self._makeUTCSaveDF(
                            ...
                            events,
                            ssArrayTD)
                    if self._evalTriggerCondition(
                            row, name, threshold):  # Trigger Condition
                        Sar = self._CreateCoeffArray(
                            row,
                            name,
                            threshold,
                            sta,
                        ...
                        if len(Sar) > 300:
                            detex.log(
                                __name__,
                                'over 300 events found in single continuous data chunk, perphaps minCoef is too low?',
                                level='warning')
                        if len(Sar) > 0:
            ...
            detex.log(
                __name__, '%s detections on %s completed, %d potential detection(s) recorded' %
                (detType, sta, len(DF) + numdets), pri=True)
        return histdic
    ...
    def _CorConDat(
        ...
        ssArrayTD, ssArrayFD, reqlen, offsets, mags, eventWaveForms, events, WFU, UtU = self._loadMPSubSpace(
            DFsta, self.continuousDataLength, sta, channels, samplingRate, returnFull=True, PKS=staPksDf)
        conDatGen = self.cfetcher.getConData(
            stakey,
            utcstart=self.UTCstart,
            utcend=self.UTCend,
            returnTimes=True)
        ...
        for st, utc1, utc2 in conDatGen:  # loop through each chunk of continuous data
            if st is None or len(st) < 1:
                ...
                continue
            CorDF, MPcon, ConDat = self._getRA(
                ssArrayTD, ssArrayFD, st, len(channels), reqlen, contrim, Names)
            if not isinstance(
                    CorDF, pd.DataFrame):  # if something is broken skip hours
        ...
        detType = 'Subspaces' if self.issubspace else 'Singletons'
        detex.log(
            __name__, '%s on %s completed, %d potential detection(s) recorded' %
            (detType, sta, len(DF) + numdets), pri=1)
        if self.calcHist:
            return histdic
    ...
    def _makeUTCSaveDF(
        ...
        if any(inUTCs):
            Th = threshold[name]
            of = offsets[name]
            seri = pd.Series([sta, name, Th, of, TS1, TS2, self.UTCSaves[inUTCs], MPcon], index=[
                             'Station', 'Name', 'Threshold', 'offset', 'TS1', 'TS2', 'UTCSaves', 'MPcon'])
    ...
    def _loadMPSubSpace(
        ...
        if 'UsedSVDKeys' in DFsta.columns and self.issubspace:  # test if input TRDF row is subspace
            ssArrayTD, rele, ssArrayFD, offsets, mags, eventWaveforms, eves, WFU, UtUdict, pwave, swave = {
            }, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}  # intitialize empty dicts
            if not all([isinstance(x.UsedSVDKeys, list)
                        for num, x in DFsta.iterrows()]):
                raise Exception(
                    'SVD not defined, run SVD on subspace stream class before calling detex')
            if not all([set(y) == set(DFsta.Channels[0].values()[0]) for x in DFsta.Channels for y in x.values(
            )]):  # make sure are channels are the same for all events
                detex.log(
                    __name__,
                    'all stations in subspace do not have the same channels',
                    level='warning')
                raise Exception(
                    'all stations in subspace do not have the same channels')
            ...
            for a in DFsta.iterrows():
                events = a[1].Events
                U = np.array([a[1].SVD[x]
                              for x in a[1].UsedSVDKeys])  # used SVD basis
                ...
                rele[
                    ...
                    np.max(
                        np.shape(
                            ssArrayTD[
                                a[1].Name])))
                ssArrayFD[a[1].Name] = np.array([scipy.fftpack.fft(
                    x[::-1], n=2**rele[a[1].Name].bit_length()) for x in ssArrayTD[a[1].Name]])
                offsets[a[1].Name] = a[1].Offsets
                mag = np.array([a[1].Stats[x]['magnitude'] for x in events])
                if 'Starttime' in a[
                        1].SampleTrims.keys():  # if picks were made
                    WFs = np.array([a[1].AlignedTD[x][a[1].SampleTrims['Starttime']:a[
                                   1].SampleTrims['Endtime']]for x in events])
                else:
...
#                    events=np.delete(events,delvals)
                ...
                # events projected into subspace
                WFU[a[1].Name] = np.dot(WFs, UtU)
            ...
            Nc = len(channels)
            for a in DFsta.iterrows():
                if len(a[1].SampleTrims.keys()
                       ) < 1:  # skip hours where no trim times are defined
                    continue
                events = a[1].Events
                Upre = [
                    np.array(
                        a[1].MPtd.values()[0][
                            a[1].SampleTrims['Starttime']:a[1].SampleTrims['Endtime']])]
    ...
    def _getPandSWaves(self, PKS, a, events):
        ...
        if isinstance(PKS, pd.DataFrame):
            for eve in events:
                ...
                if len(pkeve) < 1 or pkeve.iloc[
                        0].S == 0:  # if current event not found in picks files
                    Pwaves.append(-999)
                    Swaves.append(-999)
                else:
                    pk = pkeve.iloc[0]
                    ...
                    Pend = pk.S - pk.P
                    Psamps = [int(a[1].SampleTrims['Starttime'] +
                                  round(Poffset *
                                        stat['sampling_rate']) *
                                  stat['Nc']), int(round(Pend *
                                                         stat['sampling_rate']) *
                                                   stat['Nc'])]
                    Pwaves.append(
                        a[1].AlignedTD[eve][
...
#                    plt.show()
        ...
        while Ceval.max() >= threshold[name]:
            ...
            if self.extrapolateTimes:  # extrapolate times
                times = self._subSamp(
                    ...
                    CorSeries.SampRate,
                    CorSeries.TimeStamp)
                times1 = float(trigIndex) / CorSeries.SampRate + \
                    CorSeries.TimeStamp
                if abs(times1 - (times)) > 1.0 / CorSeries.SampRate:
                    detex.log(
                        __name__,
                        'subsample extrapolation shifts time more than one sample',
                        level='warning')
                    # make sure subsample extrapolation doesnt shift more than
                    # 1  sample
                    raise Exception(
                        'subsample extrapolation shifts time more than one sample')
            else:
            ...
            Ceval = self._downPlayArrayAroundMax(
                Ceval, CorSeries.SampRate, dpv)

            if self.estimateMags:  # estimate magnitudes
                ProEnmag, stdMag, SNR = self._estimateMagnitude(trigIndex, CorSeries, MPcon, mags[
                                                                name], events[name], WFU[name], UtU[name], eventWaveForms[name], coef)
            ...
            if count > 4000:
                detex.log(
                    __name__,
                    ' _CreatCoeffArray loop exceeds limit of 4000 events in one continuous data chunk',
                    level='warning')
                raise Exception(
                    ' _CreatCoeffArray loop exceeds limit of 4000 events in one continuous data chunk')
            MSTAMPmax, MSTAMPmin = times - \
                np.min(offsets[name]), times - np.max(offsets[name])
            Sar.loc[count] = [
                coef,
                SLValue,
                times,
    ...
    def _estimateArrivals(
        ...
        for evenum, eve in enumerate(pwave):
            if isinstance(eve, np.ndarray):
                ...
                CCses.append(CCs)
                ind = CCs.argmax()
                stmp = self._getArrivalStartTimes(
                    ind, CorSeries, trigIndex, WFlen)
                Pccs[evenum, 0] = CCs.max()
                Pccs[evenum, 1] = stmp
        deb(CCses)

    def _getArrivalStartTimes(self, ind, CorSeries, trigIndex, WFlen):
        return (trigIndex - WFlen + ind) * \
            (CorSeries.SampRate / CorSeries.Nc) + CorSeries.TimeStamp

        """
        basic function to get the corresponding time stamp for the max arrival times
        """

    def _estimateMagnitude(
            self,
            trigIndex,
            CorSeries,
            MPcon,
        ...
        # Calc magnitudes, use projected energy estimates (similar to waveform
        # scaling) and
        # only use magnitude if it is greater than -15 (-999 is often default
        # if magnitude was not calculated)
        touse = mags > -15
        if self.issubspace:
            if not any(touse):  # if no defined magnitudes avaliable
                projectedEnergyMags, stdMags = np.NaN, np.Nan
            else:
                eventCors = np.array([fast_normcorr(x, ConDat)[0] for num, x in enumerate(
                    eventWaveForms)])  # correlation coefs between each event and continuous data
                projectedEnergyMags = np.sum([(mags[x] + np.log10(np.sqrt(proEn[x]))) * np.square(
                    eventCors[x]) for x in range(len(proEn)) if mags[x] > -10]) / np.sum(np.square(eventCors[touse]))
                stdMags = np.sum([(mags[x] + np.log10(np.std(ConDat) / np.std(eventWaveForms[x]))) * np.square(
                    eventCors[x]) for x in range(len(proEn)) if mags[x] > -10]) / np.sum(np.square(eventCors[touse]))
        else:
            # make sure if single is being used only one magnitude is recorded
            assert len(mags) == 1
            if np.isnan(mags[0]) or mags[0] < -15:
                ...
                projectedEnergyMags = mags[
                    0] + np.log10(np.dot(ConDat, WFU[0]) / np.dot(WFU[0], WFU[0]))
    ...
    def _subSamp(self, Ceval, trigIndex, sr, starttime):
        ...
        if trigIndex != ind:
            detex.log(
                __name__,
                'something is messed up, trigIndex and CC.argmax no equal',
                level='warning')
            raise Exception(
                'something is messed up, trigIndex and CC.argmax no equal')
        if ind == 0 or ind == len(
                Ceval) - 1:  # If max occurs at beg or end of CC set as beg or end, no extrapolation
            tau = float(ind) / sr + starttime
        else:
            alpha = np.arccos(
                (Ceval[ind - 1] + Ceval[ind + 1]) / (2 * Ceval[ind]))
            tau = -(np.arctan((Ceval[ind - 1] - Ceval[ind + 1]) / (2 * Ceval[ind]
                                                                   * np.sin(alpha))) / alpha) * 1.0 / sr + ind * 1.0 / sr + starttime
            if -np.arctan((Ceval[ind - 1] - Ceval[ind + 1]) /
                          (2 * Ceval[ind] * np.sin(alpha))) / alpha > 1:
                detex.log(
                    __name__,
                    'Something wrong with extrapolation, more than 1 sample shift predicted ',
                    level='warning')
                raise Exception(
                    'Something wrong with extrapolation, more than 1 sample shift predicted ')
        return tau

    # Replace where Nans occur with closet non-Nan value
    ...
    # function to zero out correlation arrays around where max occurs
    ...
    def UTCfitin(self, yearrange, jdayrange, UTCstart, UTCend):
        ...
        for a in range(len(yearrange)):
            for b in range(len(jdayrange[a])):
                UTCrep = obspy.core.UTCDateTime(
                    year=int(
                        yearrange[a]), julday=int(
                        jdayrange[a][b]))
                if UTCrep.timestamp <= UTCend.timestamp and UTCrep.timestamp >= UTCstart:
                    jdrangeOut[a].append(jdayrange[a][b])
        return jdrangeOut

    # Function to take CC coefficients and rolling std matricies and calculate
    # combined coeficients
    def _standardizeCorrelation(self, R):
    ...
    # Function to load templates, also returns channels found in template
    def _getTemplate(self, EveDir, eve, sta):
        eventSubDir = glob.glob(
            os.path.join(
                EveDir,
                eve,
                sta,
                '*'))[0]  # Length will only be 1
        chans = [os.path.basename(x).split('.')[2].split(
            '_')[0] for x in glob.glob(os.path.join(eventSubDir, '*'))]
        tem = [0] * len(chans)
        for a in range(len(chans)):
            tem[a] = obspy.core.read(
                os.path.join(
                    eventSubDir,
                    '*' +
                    chans[a] +
    ...
    def _getcontinuousRanges(self, ConDir, sta, UTCstart, UTCend):
        # get lists of all hours and days for which continuous data is
        # avaliable
        conrangeyear = [
    ...
    def _getAdjustedTimes(self, templatePath):
        ...
        for waveform in glob.glob(os.path.join(Templates, '*')):
            ...
            if len(
                    trim) == 0:  # Look for trim file (tms) if not found then simply use trace
                timeStarts[
                    TR[0].stats.network +
                    '.' +
                    TR[0].stats.station] = TR[0].stats.starttime.timestamp
            else:
                timeStarts[
                    TR[0].stats.network +
                    '.' +
                    TR[0].stats.station] = trim[0]
    ...
    # Make sure all channels of each station have equal starttimes
    def _checkTraceStartTimes(self, TR):
        ...
        for a in range(len(TR)):
            ...
            if Tn > 2 * sr + T0 or Tn < -sr * 2 + T0:
                detex.log(
                    __name__,
                    'Time stamps not equal for all channels of ' +
                    TR[0].stats.station,
                    level='warning')
                raise Exception(
                    'Time stamps not equal for all channels of ' +
    ...
    def _getFilesToCorr(self, sta, jday, year, chans, ConDir):
                                # Get unique hours and corresponding files for
                                # all components
        FilesToCorr = []
        for a2 in range(len(chans)):
            FilesToCorr = FilesToCorr + \
                glob.glob(os.path.join(ConDir, sta, str(year), str(jday), chans[a2], '*'))
        sharedHours = list(set([os.path.basename(x).split(
            '-')[1].split('.')[0].split('T')[1] for x in FilesToCorr]))
        FilesToCorr = [0] * len(chans)
        for a2 in range(len(chans)):
            FilesToCorr[a2] = [0] * len(sharedHours)
            for a3 in range(len(sharedHours)):
                # FilesToCorr is now a len(chans) by len(sharedHours) matrix
                # populated with paths to continuous waveforms
                FilesToCorr[a2][a3] = glob.glob(
                    os.path.join(
                        ConDir,
                        sta,
                        year,
    ...
    def _getRA(self, ssArrayTD, ssArrayFD, st, Nc, reqlen, contrim, Names):
        ...
        try:
            conStream = detex.construct._applyFilter(
                st, self.filt, self.decimate, self.dtype, fillZeros=self.fillZeros)
        except:
            #            #detex.deb([st,self.filt,self.decimate,self.dtype,self.fillZeros])
            detex.log(
                __name__,
                'failed to filter %s, skipping' %
                st,
                level='warning')
#            return None,None,None

        CorDF.SampRate = conStream[0].stats.sampling_rate
        MPcon, ConDat, TR = multiplex(
            conStream, Nc, returnlist=True, retst=True)
        ...
        if ctrim < 0:  # Trim continuous data to avoid overlap
            ...
            MPconcur = MPcon
        MPconFD = scipy.fftpack.fft(
            MPcon, n=2**int(np.median(reqlen.values())).bit_length())

        for name, row in CorDF.iterrows(
        ):  # loop through each hour in this Jday and calculate statistic
            if len(MPcon) <= np.max(np.shape(ssArrayTD[
                    name])):  # make sure the template is shorter than continuous data else skip
                MPcon = None
            if isinstance(
                    MPcon,
                    np.ndarray):  # If channels not equal in length and multiplexing fails delete hour, else continue with CCs
                ssd = self._MPXDS(
                    MPconcur,
                    reqlen[name],
                    ssArrayTD[name],
...
#                    return None,None,None
    ...
    def _MPXDS(self, MPcon, reqlen, ssArrayTD, ssArrayFD, Nc, MPconFD):
        #        MPconFD=scipy.fftpack.fft(MPcon,n=2**reqlen.bit_length())
        n = np.int32(np.shape(ssArrayTD)[1])  # length of each basis vector
        # rolling mean of continuous data
        a = pd.rolling_mean(MPcon, n)[n - 1:]
        b = pd.rolling_var(MPcon, n)[n - 1:]  # rolling var of continuous data
        b *= n  # rolling power in vector
        # the sume of all the subspace basis vectors
        sum_ss = np.sum(ssArrayTD, axis=1)
        av_norm = np.multiply(
            a.reshape(
                1, len(a)), sum_ss.reshape(
                len(sum_ss), 1))  # term to account for non-averaged vectors
