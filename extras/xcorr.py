...
def getFAS(numConDat, calcReverse=False, tempKeyPath='TemplateKey.csv', EveDir='EventWaveForms', ConDir='ContinousWaveForms', filt=[1, 10, 2, True], decimate=False,
    ...
    fas = FAS(numConDat, calcReverse, tempKeyPath, EveDir, ConDir, filt, decimate,
              templateDuration, saveit, filename, pks, secB4FirstArrival, LTATime, STATime, staltalimit)
...
class FAS(object):
    ...
    """
    # TODO remove all methods that are not to be called after calculating the
    # FAS, no reason to pack them all into FAS class
    ...
    def __init__(self, numConDat, calcReverse, tempKeyPath, EveDir, ConDir, filt, decimate, templateDuration, saveit, filename, pks, secB4FirstArrival, LTATime, STATime, staltalimit):
        ...
        for a in self.tempKey.iterrows():  # Loop through each tempalte
            ...
            df = pd.DataFrame(index=[a[1].NAME], columns=[
                              x.NETWORK + '.' + x.STATION for num, x in staKey.iterrows()])
            for b in staKey.iterrows():  # loop through each station
                chans = b[1].CHANNELS.split('-')

                toread = glob.glob(os.path.join(self.EveDir, a[1].NAME, b[
                                   1].NETWORK + '.' + b[1].STATION + '*'))
                if len(toread) < 1:
                    continue
                template = self._applyFilter(obspy.core.read(toread[0]), filt)
                # template,trimTime=self._loadTemplate(a[1],b[1]) #load
                # template waveform for given station/event

                Nc = len(list(set([x.stats.channel for x in template])))
                jdays = self._mapjdays(a[1], b[1])
                # make sure there are at least conDatNum samples avaliable
                if numConDat > len(jdays) * (3600 / ContinousDataLength) * 4:
                    print 'Not enough continous data for conDatNum=%d, decreasing to %d' % (numConDat, len(jdays) * (3600 / ContinousDataLength) * 4)
                    numConDat = int(
                        len(jdays) * (3600 / ContinousDataLength) * 4)
                CCmat = [0] * numConDat
                # multplexed template
                MPtem = self._multiplex(template, Nc, Template=True)
                if calcReverse:
                    # Reverse order of MPtem if calculating reverse
                    MPtem = MPtem[::-1]
                reqlen = int(len(MPtem) + Nc * ContinousDataLength *
                             template[0].stats.sampling_rate - 1)
                # multplexed template in frequency domain
                MPtemFD, sum_nt = self._FFTemplate(MPtem, reqlen)
                usedJdayHours = []
                for c in range(numConDat):
                    condat, usedJdayHours = self._loadRandomContinousData(
                        jdays, filt, usedJdayHours, LTATime, STATime, staltalimit)
                    # condat=self._loadContinousData(a[1],b[1],jdays)
                    MPcon = self._multiplex(condat, Nc)
                    CCmat[c] = self._MPXCorr(
                        MPcon, MPtemFD, reqlen, sum_nt, MPtem, Nc)
                CCs = np.fromiter(itertools.chain.from_iterable(
                    CCmat), dtype=np.float64)
                # deb(CCs)
                ...
                betaparams = scipy.stats.beta.fit(
                    (CCs + 1) / 2, floc=0, fscale=1)  # transformed beta
                # enforce hard limits on detection statistic
                outdict['betadist'] = betaparams
    ...
    # loads random chunks of data from total availible data
    def _loadRandomContinousData(self, jdays, filt, usedJdayHours, LTATime, STATime, staltalimit):
        failcount = 0
        while failcount < 50:
            ...
            try:
                ...
                cft = obspy.signal.trigger.classicSTALTA(TRz[0].data, STATime * TRz[0].stats.sampling_rate, LTATime * TRz[
                                                         0].stats.sampling_rate)  # make sure no high amplitude signal is here
    ...
    def _multiplex(self, TR, Nc, Template=False, trimTolerance=15, returnlist=False):
        # find starttime for each trace
        startime = [x.stats.starttime for x in TR]
        ...
        if max(minlen) - min(minlen) > 15:
            if Template:
                raise Exception(
                    'timTolerance exceeded, examin Template: \n%s' % TR[0])
            else:
                print(
                    'timTolerance exceeded, examin data, Trimming anyway: \n%s' % TR[0])
                trimDim = min(minlen)
    ...
    def _MPXCorr(self, MPcon, MPtemFD, reqlen, sum_nt, MPtem, Nc):
        ...
        b *= np.sqrt((n - 1.0) / n)
        c = scipy.real(scipy.fftpack.ifft(np.multiply(MPtemFD, MPconFD))[
                       len(MPtem) - 1:len(MPcon)])
        result = (c - sum_nt * a) / b
        return result[::Nc]
    ...
    # Apply a filter/decimateion to an obspy trace object and trim
    def _applyFilter(self, Trace, filt, trimtime=None):
    ...
    def _mapjdays(self, template, station):
        years = glob.glob(os.path.join(
            self.ConDir, station.NETWORK + '.' + station.STATION, '*'))
        jdays = [0] * len(years)
    ...
    def _loadTemplate(self, template, station):  # load, trim and filter tempalte
        toread = glob.glob(os.path.join(
            self.EveDir, template.NAME, station.NETWORK + '.' + station.STATION + '*'))
        ...
        if self.filt != None:
            TR.filter('bandpass', freqmin=self.filt[0], freqmax=self.filt[
                      1], corners=self.filt[2], zerophase=self.filt[3])
        if self.templateDuration == None:
            TR = TR.slice(starttime=obspy.core.UTCDateTime(
                trimtime[0]), endtime=obspy.core.UTCDateTime(trimtime[1]))
        else:
            TR = TR.slice(starttime=obspy.core.UTCDateTime(
                trimtime[0]), endtime=obspy.core.UTCDateTime(trimtime[0]) + self.templateDuration)
            TR.detrend('linear')
        return TR, trimtime
...
class Detex(object):
    ...
    def __init__(self, trigCon, trigParameter, tempKeyPath, ConDir, EveDir, pks, CorDB, FASfile, UTCstart, UTCend, triggerLTATime, triggerSTATime, decimate, filt,
        ...
        self.ContinousDataLength = detex.util.getContinousDataLength(
            Condir=ConDir)
        ...
        # Creates a seperate table in Corrs.db and saves the filter params if
        # used. Necesary for CorResults class later
        self._saveFiltParams()
        eveDF = pd.DataFrame()
        for evenum, eve in self.tempKey.iterrows():  # Iterate through each event
            ...
            for sta in Stations:  # itterate through each station and create dataframe so no template data has to be accsessed later
                ...
                if len(wf) == 0 or len(wf) > 1:
                    print ('Either no file found for Event %s station %s or multiple files found, skipping. Check %s' % (
                        eve.NAME, sta, self.EveDir))
                elif len(wf) == 1:  # if the file exists
                    try:
                        pk = self.pksDF[(self.pksDF.Name == eve.NAME) & (
                            self.pksDF.Station == sta)].iloc[0]
                    except IndexError:
                        raise Exception(
                            '%s on station %s is not in the pks file, run detex.trimTemplates again' % (eve.NAME, sta))
                    # first arrivial minus origin time
                    offset = obspy.core.UTCDateTime(
                        pk.Starttime) - self.secB4FirstArrival - obspy.core.UTCDateTime(eve.TIME)
                    # calculate the threshold for this event
                    threshold = self._getThreshold(eve.NAME, sta)
...
#                    tr=obspy.read(wf[0])
                    ...
                    reqlen = int(
                        len(MPtem) + Nc * self.ContinousDataLength * TR[0].stats.sampling_rate - 1)
                    ...
                    # not the best way to do this, try harder next time
                    eveDF = eveDF.append(df, ignore_index=True)
        eveDF.reset_index(inplace=True, drop=True)
        self.histDF = self._makeHistDF(eveDF)
        if len(eveDF) < 1:
            raise Exception(
                'event dataframe empty, make sure template key and station key are correct and the waveforms are downloaded')
        jobs = []
        for stanum, starow in staKey.iterrows():  # loop throuogh each station and perform all correlations
            if multiprocess == True:
                p = multiprocessing.Process(
                    target=self._CorStations(eveDF, starow))
                jobs.append(p)
                p.start()
            else:
    ...
    # TODO rearrange loops so continous data is only read in once and all
    # templates operate before next read in=huge IO savings
    def _CorStations(self, eveDF, starow):
        sta = starow.NETWORK + '.' + starow.STATION
        DF = pd.DataFrame()  # Initialize emptry data frame
        conrangejulday, conrangeyear = self._getContinousRanges(
            self.ConDir, sta, self.UTCstart, self.UTCend)
        if len(conrangeyear) < 1:
            print(
                'No files found for station %s, make sure they exist in the continous data directory' % sta)
        ...
        for a in range(len(conrangeyear)):  # for each year
            for b in conrangejulday[a]:  # for each julian day
                FilesToCorr = glob.glob(os.path.join(
                    self.ConDir, sta, str(conrangeyear[a]), str(b), '*'))
                for ftc in FilesToCorr:
                    CorDF, MPcon = self._getRA(ftc, evedf, sta)
                    if self.averageAllTemplates:
                        CorDF = self._getAverageCorDF(CorDF)
                    # if any aspect of CorDF failed, skip hour
                    if not isinstance(CorDF, pd.DataFrame):
                        continue
                    for coreve, corrow in CorDF.iterrows():  # Loop each shared hour, or row of Tstamp and Results
                        # Trigger Condition
                        if self._evalTriggerCondition(corrow):
                            Sar = self._CreateCoeffDF(
                                corrow, coreve, evedf, MPcon)
                            if len(Sar) > 300:
        ...
        if len(DF) > 0:
            detex.util.saveSQLite(DF, self.CorDB, 'cor_df')
        print ('Correlations for %s completed, %d potential detection(s) recorded' % (
            sta, 1000 * dfCompletes + len(DF)))
    ...
    # function to make sure given trigcons and trigParameters will work and
    # have all required parts
    def _validateTrigCon(self):
        if not os.path.exists(self.pks):
            raise Exception(
                '%s not found, a pks file is required to run correlations. Create it by calling detex.util.trimTemplates' % self.pks)
        if self.trigCon == 2 or self.trigCon == 3:
            if not os.path.exists(self.FASfile):
                raise Exception(
                    '%s not found, a FAS file is required to when trigCon = 2  or 3. Create it by calling detex.xcorr.getFAS' % self.FASfile)
        if self.trigCon == 0:
            if not isinstance(self.trigParameter, numbers.Number) or abs(self.trigParameter) > 1:
                raise Exception(
                    'When trigCon = 0 trigParameter is the required correlation coeficient and therefore must be a number between 0 and 1')
        elif self.trigCon == 1:
            if not isinstance(self.trigParameter, numbers.Number) or self.trigParameter < 1:
                raise Exception(
                    'When trigCon = 1 triparameter is the sta/lta of the correlation coeficients and therefore must be a number greater than 1')

        elif self.trigCon == 2:
            if not isinstance(self.trigParameter, numbers.Number) or self.trigParameter < 0 or self.trigParameter > 1:
                raise Exception(
                    'trigParameter must be a likelihood (between 0 and 1, usually a number very close to 0) when trigCon=2')
        elif self.trigCon == 3:
            ...
            raise Exception(
                'trigCon not supported, see docs for supported values')
    ...
    def _getThreshold(self, event, station):
        if self.trigCon == 0 or self.trigCon == 1:
            ...
            if self.trigCon == 2:
                ...
                thresh = scipy.stats.beta.isf(self.trigParameter, betaDist[
                                              0], betaDist[1]) * 2 - 1
                thresh = scipy.stats.norm.isf(self.trigParameter, fas['normparams'][
                                              0], fas['normparams'][1])
            if self.trigCon == 3:
                thresh = fas['normparams'][1] * \
                    self.trigParameter + fas['normparams'][0]
        return thresh
    ...
    # Apply a filter/decimateion to an obspy trace object and trim
    def _applyFilter(self, Trace, trimtime=None, condat=False):
        ...
        if self.templateDuration and condat:  # trim off the bit of extra cotninous waveform #TODO Make this work with varaible template sizes
            starttime, endtime = min([x.stats.starttime for x in Trace]), max(
                [x.stats.endtime for x in Trace])
            ...
            if endtime - secToTrim < starttime:
                ...
                return None
            Trace = Trace.slice(starttime=starttime,
                                endtime=endtime - secToTrim)
       # Trace.sort()
        # Trace.detrend()
        return Trace
...
#            TR.decimate(self.decimate)
        if self.templateDuration == None:
            pass
            TR.trim(starttime=obspy.core.UTCDateTime(
                trimtime[0]), endtime=obspy.core.UTCDateTime(trimtime[1]))
        else:
            TR.trim(starttime=obspy.core.UTCDateTime(trimtime[
                    0]), endtime=obspy.core.UTCDateTime(trimtime[0]) + self.templateDuration)
            TR.detrend('linear')
        return TR, trimtime
    ...
    def _saveFiltParams(self):
        if self.filt:
            if len(self.filt) != 4:  # Make sure valid parameters are passed
                raise Exception(
                    'Bad Filter Parameters, see docs for correct format')
            ...
            if self.decimate and not(isinstance(self.decimate, int)):
                raise Exception(
                    'Bad decimation value, must either be None or an integer (generally less than 10)')
            # deb([self.filt,self.decimate])
            df = pd.DataFrame([self.filt + [self.decimate]], columns=['FREQMIN',
                                                                      'FREQMAX', 'CORNERS', 'ZEROPHASE', 'DECIMATE'], index=[0])
    ...
    def _calcAlpha(self, trigIndex, MPcon, MPtem, Nc):
        ...
        for a in range(10):
            ...
            alpha = np.dot(Y, np.multiply(X, Weight)) / \
                np.dot(X, np.multiply(X, Weight))
            # If change is less than 1% break, else go 10 iterations
            if (abs(alpha - alphaPrevious)) / (alphaPrevious) < 0.01:
                break
            alphaPrevious = alpha
        #print (X.argmax(),Y.argmax(),alpha)
        return alpha

    # creates a datafram with necesariy info about each detection
    def _CreateCoeffDF(self, corrow, coreve, evdf, MPcon):
        ...
        alpha = pd.Series(self._calcAlpha(
            trigIndex, MPcon, everow.MPtem, everow.Nc))
        coef = pd.Series(corrow.Xcor[trigIndex])
        if self.extrapolateTimes:
            times = pd.Series(self._subsampleExtrapolate(
                Ceval, trigIndex, corrow.SampRate, corrow.TimeStamp))
            times1 = [float(trigIndex) / corrow.SampRate +
                      corrow.TimeStamp + self.secB4FirstArrival]
        ...
        # This doesnt seem to be working, exmain when you can #################
        ...
        while Ceval.max() >= corrow.threshold:
            ...
            if self.extrapolateTimes:
                times[len(times)] = self._subsampleExtrapolate(
                    Ceval, trigIndex, corrow.SampRate, corrow.TimeStamp)
                times1 = [float(trigIndex) / corrow.SampRate +
                          corrow.TimeStamp + self.secB4FirstArrival]
            ...
            alpha[len(alpha)] = self._calcAlpha(
                trigIndex, MPcon, everow.MPtem, everow.Nc)
            Ceval = self._downPlayArrayAroundMax(Ceval, corrow.SampRate, dpv)
            count += 1
            if count > 4000:
        ...
        Sar = pd.DataFrame()
        Sar['Coef'], Sar['STA_LTACoef'], Sar['STMP'], Sar[
            'alpha'] = coef, SLValue, times, alpha
        Sar['Template'] = coreve
    ...
    def _makeDataFrame(self, eve, sta, Sar, offSetDict):
        Sar['Template'], Sar['Sta'], Sar['MSTMP'] = eve, sta, [
            x - offSetDict[sta] for x in Sar['STMP'].tolist()]
        return Sar
    ...
    def _subsampleExtrapolate(self, Ceval, trigIndex, sr, starttime):
        ...
        # If max occurs at beg or end of CC set as beg or end, no extrapolation
        ...
        for a in range(len(yearrange)):
            for b in range(len(jdayrange[a])):
                UTCrep = obspy.core.UTCDateTime(
                    year=int(yearrange[a]), julday=int(jdayrange[a][b]))
    ...
    def _getTemplate(self, EveDir, eve, sta):
        eventSubDir = glob.glob(os.path.join(
            EveDir, eve, sta, '*'))[0]  # Length will only be 1
        chans = [os.path.basename(x).split('.')[2].split(
            '_')[0] for x in glob.glob(os.path.join(eventSubDir, '*'))]
        tem = [0] * len(chans)
        for a in range(len(chans)):
            tem[a] = obspy.core.read(os.path.join(
                eventSubDir, '*' + chans[a] + '*.sac'))
        global debug
        return chans, tem
    ...
    def _getContinousRanges(self, ConDir, sta, UTCstart, UTCend):
        ...
        conrangeyear = [os.path.basename(x) for x in glob.glob(
            os.path.join(ConDir, sta, '*'))]
        conrangejulday = [0] * len(conrangeyear)
        for a in range(len(conrangeyear)):
            conrangejulday[a] = [os.path.basename(x) for x in glob.glob(
                os.path.join(ConDir, sta, conrangeyear[a], '*'))]
        # Trim conrangeyear and conrangejulday to fit in UTCstart and UTCend if
        # defined
        if UTCstart is not None and UTCend is not None:
    ...
    def _checkTraceStartTimes(self, TR):
        ...
        for a in range(len(TR)):
            ...
            if Tn > 2 * sr + T0 or Tn < -sr * 2 + T0:
                raise Exception(
                    'Time stamps not equal for all channels of ' + TR[0].stats.station)
    ...
    def _getFilesToCorr(self, sta, jday, year, chans, ConDir):
        ...
        for a2 in range(len(chans)):
            FilesToCorr = FilesToCorr + \
                glob.glob(os.path.join(ConDir, sta, str(
                    year), str(jday), chans[a2], '*'))
        ...
        for a2 in range(len(chans)):
            ...
            for a3 in range(len(sharedHours)):
                ...
                # populated with paths to continous waveforms
                FilesToCorr[a2][a3] = glob.glob(os.path.join(ConDir, sta, year, jday, chans[
                                                a2], '*' + 'T' + sharedHours[a3] + '.sac'))
        return FilesToCorr, sharedHours

    def _getRA(self, ftc, evedf, sta):
        CorDF = pd.DataFrame(index=evedf.Event.values, columns=[
                             'Xcor', 'STALTA', 'TimeStamp', 'SampRate', 'MaxCC', 'MaxSTALTA', 'threshold', 'Nc'])
        ...
        MPconFD = scipy.fftpack.fft(
            MPcon, n=2 ** int(evedf.reqlen.median()).bit_length())
        # TODO This assumes all templates are of equal length, if not will
        # break
        n = int(np.median([len(x) for x in evedf.MPtem]))
        a = pd.rolling_mean(MPcon, n)[n - 1:]
        b = pd.rolling_std(MPcon, n)[n - 1:]
        b *= np.sqrt((n - 1.0) / n)
        ...
        for corevent, corrow in CorDF.iterrows():
            ...
            # make sure the template is shorter than continous data else skip
            if len(MPcon) <= (len(evrow.MPtem)) + self.triggerLTATime * corrow.SampRate:
                return None, None
            # If channels not equal in length and multiplexing fails delete
            # hour, else continue with CCs
            if isinstance(MPcon, np.ndarray):
                try:
                    CorDF.Xcor[corevent] = self._MPXCorr(
                        MPcon, MPconFD, evrow.MPtemFD, evrow.reqlen, evrow.sum_nt, evrow.MPtem, evrow.Nc, n, a, b)
                except:
                    deb([corevent, evrow, n, a, b, CorDF, MPcon, MPconFD])
                CorDF.MaxCC[corevent] = CorDF.Xcor[corevent].max()
                CorDF.STALTA[corevent] = self._getStaLtaArray(CorDF.Xcor[
                                                              corevent], self.triggerLTATime * corrow.SampRate, self.triggerSTATime * corrow.SampRate)
            ...
            if self.calcHist:  # calculate histograms
                ...
                self.histDF.loc[corevent, sta]['hist'] = self.histDF.loc[corevent, sta][
                    'hist'] + np.histogram(CorDF.Xcor[corevent], bins=self.histBins)[0]
                self.histDF.loc[corevent, sta]['histSTALTA'] = self.histDF.loc[corevent, sta][
                    'histSTALTA'] + np.histogram(CorDF.STALTA[corevent], bins=self.histBins)[0]
        return CorDF, MPcon
    ...
    # multiplex normalized cross correlation
    def _MPXCorr(self, MPcon, MPconFD, MPtemFD, reqlen, sum_nt, MPtem, Nc, n, a, b):
        ...
        c = scipy.fftpack.ifft(np.multiply(MPtemFD, MPconFD))[
            len(MPtem) - 1:len(MPcon)]
        result = (c - sum_nt * a) / b
        # result[np.where(np.isinf(result))]=0.0 # when filling missing values
        # with 0 infs can occur, change to 0 because they are mwaningless
        # anyway
    ...
    # function to find longest continous data chucnk and discard the rest
    def _mergeChannels(self, TR):
        ...
        TR.trim(starttime=TR[lemax].stats.starttime,
                endtime=TR[lemax].stats.endtime)
        return TR
